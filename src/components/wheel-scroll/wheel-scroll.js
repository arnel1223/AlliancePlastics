...
import { View, ScrollView, Dimensions, Platform, TouchableOpacity, Image } from "react-native";
  ...
  componentDidMount() {
    if (typeof this.props.selectedIndex !== undefined) {
      ...
      if (this.props.onValueChange) {
        const selectedValue = this.props.dataSource[this.props.selectedIndex];
        this.props.onValueChange(selectedValue, this.props.selectedIndex);
      }
    }
  }
      ...
      >
        ...
        <View
          style={[
            ...
            {
              left: this.props.icon ? wp(30) : 0,
              top: (wrapperHeight - wp(45)) / 2,
    ...
    if (this.props.icon) {
      return (
        <TouchableOpacity
          activeOpacity={0.8}
          key={index}
          style={[styles.selectedItemRow, { height: this.props.itemHeight }]}
          onPress={() => {
        ...
        >
          <Image source={data.icon} style={[styles.itemIcon, isSelected ? { tintColor: this.props.highlightColor } : { tintColor: COLOR.PRIMARY_INPUT_COLOR}]} />
          <Text
            style={isSelected ? this.props.activeItemTextStyle : this.props.itemTextStyle}
          >
            {data.value}
